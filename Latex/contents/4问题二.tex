...
\subsection{模型稳定性分析}
为检验决策模型所得结论的稳定性，需要评估关键参数与技术标准变动对最优NIPT时点选择的影响。本节分别分析了机会成本函数中的时间延迟参数以及Y染色体浓度检测阈值这两个核心要素。

\subsubsection{机会成本参数敏感性分析}
在风险量化模型中，机会成本的计算涉及一个关键参数$\Delta t$，它代表因检测失败而必须推迟下一次检测的时间间隔。该参数的取值会直接影响失败风险的量化大小，进而改变总风险曲线的形态。为了探究模型对该参数的敏感程度，我们设定其他条件不变，调整了$\Delta t$的数值。

具体操作中，我们将$\Delta t$的取值范围设定在14天至28天之间，并在此区间内进行遍历。对于每一个$\Delta t$的取值，我们都重新计算所有分组的总期望风险曲线，并求解出新的最优NIPT检测时点。这一过程能够展示最优时点如何随失败代价的变化而做出调整。

分析结果如\cref{fig:q2_sens_delta_t}所示。图中显示，随着时间延迟参数$\Delta t$的增加，多数群组的最佳检测时点均向更早的孕周移动。时间延迟$\Delta t$的增加意味着检测失败将导致更高的机会成本。面对增大的失败代价，决策模型会选择一个更早的检测时间点，此时Y染色体浓度达标的预期概率更高，从而降低发生失败的可能性。
...
\subsubsection{检测标准变动影响分析}
NIPT检测的成功与否取决于Y染色体浓度是否达到临床标准，即4\%的阈值。这一技术标准可能因检测技术的发展或不同检测机构的实践而发生变动。因此，有必要分析最优时点策略对此类标准变化的依赖性。

为评估这种影响，我们对Y染色体浓度阈值进行了调整分析。我们将浓度阈值在3.5\%至4.5\%的区间内进行变动，以模拟不同的技术要求或系统性检测偏差。在每个设定的阈值水平下，我们都更新了失败概率的计算函数，并重新运行完整的时点优化流程，为每一个孕妇分组确定新的最佳检测时点。

\cref{fig:q2_sens_threshold}展示了各分组的最佳检测时点随浓度阈值变化的趋势。结果表明，当成功检测的标准变得更为严格，即浓度阈值提高时，所有分组的最佳检测时点均一致性地向后推迟。这是因为模型需要通过延长等待时间来确保胎儿的Y染色体浓度有更大概率生长至新的、更高的标准之上。此结果表明，模型给出的时点建议会根据检测标准的严格程度做出合理的适应性调整。
